...
class Explainer(BaseModel):
    ...
    def compute_shapley_values(self, sample, type = ShapleyValuesType.MARGINAL, causal_struct = None):
        if type == ShapleyValuesType.CAUSAL and not causal_struct:
            raise Exception("Causal graph has to be provided for computing Causal Shapley Values")
        ...
        print("Sigma_phi + E(fX): ", round(float(sum(list(phis.values())) + f_x), 3))
    ...
    def get_value(self, type, permutation, x, causal_struct, xi):
        ...
        for i in self.X_counter:
            ...
            if x_hat.tolist() not in baseline_check_1:
                ...
                match type:
                    case ShapleyValuesType.MARGINAL:
                        prob_x_hat = get_probability(self.X_counter, x_hat, indices_baseline, lenX)
                    case ShapleyValuesType.CONDITIONAL:
                        prob_x_hat = conditional_prob(self.X_counter, x_hat, indices, indices_baseline, lenX)
                    case ShapleyValuesType.CAUSAL:
                        prob_x_hat = causal_prob(self.X_counter, x_hat, indices, indices_baseline, causal_struct, lenX)
            ...
            if x_hat_2.tolist() not in baseline_check_2:
                ...
                match type:
                    case ShapleyValuesType.MARGINAL:
                        prob_x_hat_2 = get_probability(self.X_counter, x_hat_2, indices_baseline_2, lenX)
                    case ShapleyValuesType.CONDITIONAL:
                        prob_x_hat_2 = conditional_prob(self.X_counter, x_hat_2, indices_2, indices_baseline_2, lenX)
                    case ShapleyValuesType.CAUSAL:
